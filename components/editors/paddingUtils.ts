// Design Ref: signage-resolution §3.4.1, ui-redesign §3.5.1 — dynamic
// display height + user-set vertical padding.
// Shared padding calculation for editor and renderer.
// - `displayH` (canvas height) is parameterised since v1.2.1.
// - `paddingOverride` (v1.3) lets the slide.padding option replace the
//   automatic MIN_PAD↔MAX_PAD interpolation: when set, the same padding
//   is used for every row count and fontSize is recomputed accordingly.
//
// Auto strategy (no override):
...
//
// Override strategy:
// - All N: pad = paddingOverride, fontSize = (H - 2*pad) / (N*1.4)
  ...
  for (let n = 1; n <= 10; n++) {
    const pad =
      typeof paddingOverride === 'number'
        ? paddingOverride
        : Math.round(MAX_PAD - (MAX_PAD - MIN_PAD) * (n - 1) / 9);
...
export function calcVerticalPadding(
  ...
  // ui-redesign §3.5.1 — if the user pinned a vertical padding via the
  // slide.padding option, that value wins regardless of font size.
...
export function detectFontSize(
  ...
  const fontData = buildFontData(displayH, paddingOverride);
